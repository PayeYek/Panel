<template>
    ...
    <section class="default_container flex flex-col sm:flex-row-reverse sm:justify-between sm:items-center gap-4 mb-8">
        ...
        <ul class="flex items-center flex-wrap gap-x-2 gap-y-4 list-none text-base font-bold *:rounded-custom *:flex_center *:h-8 *:px-3 *:cursor-pointer *:border *:border-stone-700 *:text-stone-700">
            <li :class="categoryFilterState === 'all' ? '!border-normal !text-normal' : ''" @click="changeFilter('all')"> همه موارد </li>
            <li :class="categoryFilterState === 'news' ? '!border-normal !text-normal' : ''" @click="changeFilter('news')"> اخبار </li>
            <li :class="categoryFilterState === 'sell' ? '!border-normal !text-normal' : ''" @click="changeFilter('sell')"> اطلاعیه </li>
            <li :class="categoryFilterState === 'blog' ? '!border-normal !text-normal' : ''" @click="changeFilter('blog')"> وبلاگ </li>
        </ul>
    </section>

    <section :class="'mb-4 ' + gridStyle">
        <template v-for="(article, index) in filteredArticles">
            <ArticleLink v-if="type == 1" :key="index" :href="'/l/' + landSlug + '/a/' + article.slug" :class="'flex flex-col sm:flex-row rounded-custom bg-white overflow-hidden ' + borderStyle">
                <!-- image -->
                <div
            ...
            <ArticleLink v-if="type == 2" :key="index" :href="'/l/' + landSlug + '/a/' + article.slug" :class="'flex flex-col w-full flex-none overflow-hidden rounded-custom bg-white '+ borderStyle">
            ...
            <ArticleLink v-if="type == 3" :key="index" :href="'/l/' + landSlug + '/a/' + article.slug" class="flex flex-col sm:flex-row bg-white border-t first:border-t-0 py-4 first:pt-0 last:pb-0 border-dark-100">
                <!-- image -->
                <div
            ...
            <div v-if="type == 4" :key="index" class="flex flex-col sm:flex-row bg-white border-t first:border-t-0 py-4 first:pt-0 last:pb-0 border-dark-100">
                <!-- image -->
                <div
                    class="overflow-hidden md:flex-none w-full relative pt-[61%] sm:pt-0 sm:w-72 md:w-80 lg:w-[23rem] sm:flex-none mb-3 sm:mb-0 rounded-custom">
                    <img :src="article.image" :alt="article.title" class="absolute top-0 left-0 object-cover w-full h-full sm:static" />
                ...
                <div class="px-6 md:pl-8 flex flex-col sm:justify-center sm:flex-1">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <ArticleLink :href="'/l/' + landSlug + '/a/' + article.slug" class="text-sm font-bold flex items-center px-4 gap-4 text-normal">
                            <span> ادامه </span>
