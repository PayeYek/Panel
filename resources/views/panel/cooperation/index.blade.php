<x-layout.admin>
    ...
    <x-splade-table :for="$items" search-debounce="1000" striped
                    :title="__('Cooperation request list')"
                    pagination-scroll="preserve"
    >
        @cell('request', $item)
        <Link modal
              href="{{ route('panel.interaction.cooperation.edit', $item->id) }}"
        ...
        @cell('status', $item)
        @if($item->status->value === \App\Enums\CooperationStatus::Done->value)
            <span
        ...
        <x-layout.panel.more-buttons>
            ...
            <div class="py-2 first:pt-0 last:pb-0">
                <Link method="delete" confirm
                      href="{{ route('panel.interaction.cooperation.destroy', $item) }}"
