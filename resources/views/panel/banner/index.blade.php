<x-layout.admin>
    ...
    <x-splade-table :for="$items" search-debounce="1000" striped
                    :primaryLink="route('panel.marketing.banner.create')"
...
{{--                    slideover--}}
    ...
    >
        ...
        <Link
            slideover
            href="{{ route('panel.marketing.banner.edit', $item) }}"
            class="flex flex-col pe-10">
        <div class="flex gap-2">
            <img class="aspect-2 h-14 rounded shrink-0" src="{{$item->background}}"
                 alt="{{$item->title}}">
...
{{--                    <span class="text-xs">{{$item->title}}</span>--}}
                ...
                <span class="text-sm truncate max-w-60 lg:max-w-xs text-black dark:text-white">{{$item->description}}</span>
            </div>
        </div>
        </Link>
        ...
        <x-layout.panel.more-buttons>
            ...
            <div class="py-2 first:pt-0 last:pb-0">
                <Link method="delete" confirm
                      href="{{ route('panel.marketing.banner.destroy', $item) }}"
