...
>
    <template #default="{!! $scope !!}">
        <label class="block group/select " v-bind:class="{ 'pointer-events-none': select.loading }">
            @includeWhen($label, 'splade::form.label', ['label' => $label])

            <div class="relative rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm group-focus-within/select:ring-1 group-focus-within/select:ring-primary-500 group-focus-within/select:border-primary-500 transition duration-200">
...
//                        'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 disabled:opacity-50',
                        'rounded-[7px] min-h-[2.5rem] px-3 block bg-gray-50 dark:bg-gray-700 w-full border-transparent focus:border-transparent focus:outline-none focus:ring-0 dark:placeholder-gray-400 disabled:opacity-50 disabled:bg-gray-50 disabled:cursor-not-allowed',
                    ])->merge([
                        'multiple' => $multiple,
                        'name' => $name,
                ...
                <div class="absolute inset-0 w-full h-full" v-if="select.loading">
                    <div class="flex flex-row items-center justify-center w-full h-full">
                        <svg
                            class="animate-spin me-3 h-5 w-5 text-primary-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        >
