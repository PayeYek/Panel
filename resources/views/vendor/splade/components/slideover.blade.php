<x-splade-component is="modal-wrapper" :base-attributes="$attributes->except('class')" :key="$modalKey" :close-button="$closeButton" :close-explicitly="$closeExplicitly" :name="$name">
    ...
    <div class="fixed inset-0 z-40 overflow-x-hidden overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent dark:scrollbar-thumb-gray-700 dark:scrollbar-track-transparent">
        <!-- Container to center the panel -->
        <div class="flex min-h-full items-center rtl:flex-row-reverse" v-bind:class="{
            ...
            <x-splade-component is="transition" child after-leave="modal.emitClose" animate="modal.animate" v-bind:class="{
                ...
                <x-splade-component is="dialog" panel dusk="slideover-dialog">
                    <div {{ $attributes->class('group/modal is-modal bg-white dark:bg-gray-800 p-6 min-h-screen relative font-inter rtl:font-iran text-gray-900 dark:text-white') }}>
                        <div v-if="modal.closeButton" class="absolute top-0 ltr:right-0 rtl:left-0 pt-3 pe-3 z-10">
                            <button dusk="close-modal-button" @click="modal.close" type="button" class="text-gray-400 hover:text-gray-500 dark:text-gray-600 dark:hover:text-gray-500">
