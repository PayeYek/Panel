    ...
    <tr>
        ...
        @foreach($table->columns() as $column)
            <th
                ...
                class="@if($loop->first && $hasBulkActions) pe-6 @else px-6 @endif py-3 ltr:text-left rtl:text-right font-bold tracking-wide {{ $column->classes }}"
            >
                @if($column->sortable)
                ...
                <span class="flex flex-row items-center @if($column->alignment == 'right') justify-end @elseif($column->alignment == 'center') justify-center @else justify-start @endif">
                    ...
                    @if($column->sortable)
                        <svg aria-hidden="true" class="w-3 h-3 ms-2 @if($column->sorted) text-primary-500 @else text-gray-400 @endif" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512">
                            @if(!$column->sorted)
