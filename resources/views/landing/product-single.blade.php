            ...
            <div class="flex flex-col gap-10 mt-10">
                @foreach($product->videos as $video)
                    <div
                        ...
                        <span
                            class="mt-2 mb-1 text-sm font-bold text-gray-900 dark:text-white text-center grow grid place-items-center">{{$video->alt}}</span>
                    </div>
                @endforeach
            </div>
                ...
                @foreach($product->attributes->sortBy('parent_id')->groupBy('parent_id') as $key => $attrs)
                    <x-splade-toggle :data="$loop->iteration == 1">
                        <header @click.prevent="toggle"
                                class="sticky top-[4.5rem] flex items-center justify-between transform py-4 px-5 select-none cursor-pointer bg-gray-300 dark:bg-gray-800 rounded-xl font-bold text-base">
