    ...
    <div class="md:bg-white md:drop-shadow-base {{ $radiusSize }}">
        ...
        <div class="md:w-[492px] md:mx-auto md:pt-10">
            ...
            <x-splade-form :default="['product_id' => $product->id, 'land_id' => $land->id]"
                ...
                <section
                    class="drop-shadow-base md:drop-shadow-none bg-white {{ $radiusSize }} md:rounded-none md:bg-transparent pt-6 pb-8 md:pb-11 flex flex-col items-center">

                    <div
                        class="grid grid-cols-1 md:grid-cols-2 md:gap-x-10 md:gap-y-4 gap-2 w-56 mb-6 md:w-full md:mb-9">

                        {{--                        <input name="firstName" type="text"--}}
                        {{--                               class="shadow-glass shadow-black/30 h-12 border-0 focus:ring-0 {{ $radiusSize }} outline-none w-full placeholder:text-[#888b93] text-sm font-normal pr-3 pl-6 text-gray-900"--}}
                        {{--                               placeholder="نام"/>--}}
                        {{--                        <label for="lastName" class="h-12 w-full relative">--}}
                        {{--                            <input id="lastName" required name="lastName" type="text"--}}
                        {{--                                   class="peer h-full shadow-glass shadow-black/30 border-0 focus:ring-0 {{ $radiusSize }} outline-none w-full placeholder:text-[#888b93] text-sm font-normal pr-3 pl-6 text-gray-900"--}}
                        {{--                                   placeholder="نام خانوادگی"/>--}}
                        {{--                            <i class="absolute top-2 right-20 text-red-700 peer-valid:hidden">*</i>--}}
                        {{--                        </label>--}}
                        {{--                        <label for="lastName" class="h-12 w-full relative col-span-full">--}}
                        {{--                            <input id="lastName" required name="lastName" type="text"--}}
                        {{--                                   class="peer h-full shadow-glass shadow-black/30 border-0 focus:ring-0 {{ $radiusSize }} outline-none w-full placeholder:text-[#888b93] text-sm font-normal pr-3 pl-6 text-gray-900"--}}
                        ...
                        {{--                                      class="shadow-glass shadow-black/30 h-full resize-none peer border-0 focus:ring-0 {{ $radiusSize }} outline-none w-full placeholder:text-[#888b93] text-sm font-normal pr-3 pl-6 text-gray-900"--}}
                        {{--                                      placeholder="دیدگاه (اجباری)"></textarea>--}}
                        {{--                            <i class="absolute top-1 right-[52px] text-red-700 peer-valid:hidden">*</i>--}}
                        {{--                        </label>--}}
                    </div>
                    <button type="submit"
                            class="{{ $radiusSize }} flex_center w-64 h-11 {{ $fillBtnTheme }}"> ارسال
                    </button>
                </section>
